...
Every lesson has a photograph placement, an explanatory illustration and an
interactive spatial model. Eight new contextual photographs were generated
with the built-in OpenAI image tool and saved as responsive 800/1600-pixel WebP
assets in `web/illustrations/photoreal/`. The exact prompts and generation
provenance are in `expansion-photograph-prompts.json`.

The complete collection has 30 distinct photographic scenes shared across
related lessons. They are labelled AI-generated and contextual, not unique
documentary photographs of every concept. Diagrams are authored lesson by
lesson using the existing deterministic drawing system, with complete text
...
Generated photography is an independent asset, not regenerated by these scripts.
