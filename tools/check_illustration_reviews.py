...
def audit(root=ROOT, reviews=None):
    ...
    for review in reviews:
        identifier = review["lesson"] + ("/" + review["media_id"] if review.get("media_id") else "")
    ...
    for source in sorted((root / "data/curriculum").glob("[0-9]*.json")):
        ...
        for node in curriculum["nodes"]:
            ...
            for plate_index, plate in enumerate(plates):
                ...
                record = records.pop(identifier, None)
                status = "pending" if record is None else (
                    "reviewed" if record.get("fingerprint") == fingerprint else "stale")
                rows.append({"lesson": node["id"], "media_id": plate["id"], "review_id": identifier, "domain": curriculum["id"],
    ...
    return {"lessons": len(seen_lessons), "plates": len(rows), "reviewed": sum(r["status"] == "reviewed" for r in rows),
...
if __name__ == "__main__":
    ...
    if args.json:
        ...
        print("{reviewed}/{plates} illustrations have current review records; {stale} stale".format(**report))
    ...
    raise SystemExit(int(args.require_complete and report["reviewed"] != report["plates"]))
