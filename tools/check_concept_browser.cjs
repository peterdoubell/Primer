...
const ids = process.argv[4] ? process.argv[4].split(',') : registeredIds;
assert.ok(ids.length && new Set(ids).size === ids.length && ids.every(id => registeredIds.includes(id)), 'Only registered, unique scenarios may be selected');
