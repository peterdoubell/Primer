...
def test_curriculum_visual_gallery_catalogues_every_plate_and_model_without_answers(
    ...
    expected_domain_counts = {
        ...
        'history': (29, 32), 'earth': (27, 53), 'arts': (25, 28),
        'mind': (29, 32), 'radiology': (106, 110),
    ...
    assert all(domain['items'] == domain['illustrations'] + domain['photographs'] + domain['models']
               for domain in body['domains'])
    assert all(domain['photographs'] == domain['illustrations'] for domain in body['domains'])

    expected_order = []
    for node in srv.curr.nodes.values():
        plate = next(entry for entry in node['lesson_media']
                     if entry['kind'] == 'illustration')
        expected_order.append((node['id'], plate['id'], 'illustration'))
